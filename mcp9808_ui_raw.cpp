...
#include "mcp9808_ui_raw.h"

namespace MCP9808UIFields {

class RawField : public Field<uint16_t> {
protected:
  const Register<uint8_t, uint16_t> &reg;
  const Variable<uint16_t> *var;
  MCP9808::RegisterTable &rt;

public:
  RawField(int row, const Register<uint8_t, uint16_t> *reg, MCP9808::RegisterTable &rt) :
    Field<uint16_t>(UI::statusp, row, 1, reg->NiceName(), "", ""), reg(*reg), var(NULL), rt(rt) {
      value_width = 2;
      units_width = 0;
      attributes = A_BOLD;
  }
  RawField(int row, const Register<uint8_t, uint16_t> *reg, const Variable<uint16_t> *var, MCP9808::RegisterTable &rt) :
...
using namespace MCP9808UIFields;

MCP9808UIRaw::MCP9808UIRaw(MCP9808 &mcp9808) : UI()
{
  devices.insert(&mcp9808);

  int row = 1, col = 1;
  bool first = true;
  for (auto reg : mcp9808.RT) {
    fields.push_back(new RawField(row++, reg, mcp9808.RT));
    for (auto var : *reg)
      fields.push_back(new RawField(row++, reg, var, mcp9808.RT));
    fields.push_back(new Empty(row++, col));
  }
}

MCP9808UIRaw::~MCP9808UIRaw() {}
